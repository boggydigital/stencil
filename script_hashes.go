package stencil

var ScriptHashes = []string{
	//script_iframe_size_receive_message.gohtml
	"'sha256-EoiesIg5jhsIaHn7PSaZ/oT9Yi0MCUx9WzALOyH9HkE='",
	//script_iframe_post_message.gohtml
	"'sha256-vEdzDTUjeRFG21L/pW+qldt1k+gnTSWl4v2E16iqJPc='",
	//script_onfocus_digest.gohtml
	"'sha256-n0s8aUnGgeVGVMRpMuAJ+LIRrdURaa//IaGKuDUk/WI='",
	//script_image_loading.gohtml
	"'sha256-Aj9d2Nl0niMrZy1w3VLlqviK5LNVKzscc4jrIbJO0B4='",
}
